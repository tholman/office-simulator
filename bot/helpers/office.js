...
class office {
  ...
  sheduleReaction(announcement) {
    ...
    futureDate.setDate(
      futureDate.getDate() + Math.floor(Math.random() * 10) + 2
    );
  ...
  getAnnouncement(date) {
    for (var i = 0; i < this.shedule.length; i++) {
      ...
      console.log(date, announcement.date, announcement.date < date);
      if (announcement.date < date) {
        ...
        if (this.shedule.length === 0) {
          this.sheduleItem(
            this.getRandomTimeInOfficeHours(),
            this.tasker.getRandomAction()
          );
        }

        // If it has a "reaction", shedule the reaction
        if (announcement.item.reaction) {
          this.sheduleReaction(
            this.tasker.getReaction(announcement.item.reaction)
          );
        }
  ...
  getRandomTimeInOfficeHours() {
    ...
    date.setHours(
      Math.floor(Math.random() * (this.hours.close - this.hours.open + 1)) +
        this.hours.open
    );
